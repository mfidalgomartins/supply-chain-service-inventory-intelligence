...
SLATE = colors.HexColor("#56616c")
MUTED = colors.HexColor("#9aa6b2")
...
# category service
_d = pd.read_csv(PROC / "daily_product_warehouse_metrics.csv",
                 usecols=["category", "units_demanded", "units_fulfilled",
                          "lost_sales_revenue"])
catfill = (_d.groupby("category")
           .agg(dem=("units_demanded", "sum"), ful=("units_fulfilled", "sum"),
                lost=("lost_sales_revenue", "sum")))
catfill["fill"] = catfill.ful / catfill.dem * 100
catfill = catfill.sort_values("fill")

# ABC cohort
abc = (pinv.groupby("abc_class")
       .agg(inv=("average_inventory_value", "sum"),
            lost=("lost_sales_exposure", "sum"), n=("product_id", "count"))
       .reindex(["A", "B", "C"]))
...
_md = pd.read_csv(PROC / "daily_product_warehouse_metrics.csv",
                  usecols=["date", "units_demanded", "units_fulfilled",
                           "lost_sales_revenue", "stockout_flag"],
                  parse_dates=["date"])
_md["m"] = _md["date"].dt.to_period("M")
monthly = (_md.groupby("m")
           .agg(dem=("units_demanded", "sum"), ful=("units_fulfilled", "sum"),
...
body = ParagraphStyle("body", parent=ss["BodyText"], fontName="Helvetica",
                      fontSize=10, leading=15.5, textColor=INK,
                      alignment=TA_JUSTIFY, spaceAfter=8)
lead = ParagraphStyle("lead", parent=body, fontSize=11, leading=17,
                      textColor=INK, spaceAfter=10)
h1 = ParagraphStyle("h1", parent=ss["Heading1"], fontName="Helvetica-Bold",
                    fontSize=16, leading=20, textColor=INK, spaceBefore=4,
                    spaceAfter=4)
kicker = ParagraphStyle("kicker", fontName="Helvetica-Bold", fontSize=8.5,
                        textColor=ACCENT, leading=11, spaceAfter=2)
h2 = ParagraphStyle("h2", parent=ss["Heading2"], fontName="Helvetica-Bold",
                    fontSize=12.5, leading=16, textColor=INK, spaceBefore=14,
                    spaceAfter=5)
h3 = ParagraphStyle("h3", parent=ss["Heading3"], fontName="Helvetica-Bold",
                    fontSize=10.5, leading=14, textColor=ACCENT, spaceBefore=10,
                    spaceAfter=3)
...
takeaway = ParagraphStyle("takeaway", parent=body, fontName="Helvetica-Bold",
...
def takeaway_box(text, width=CW):
    p = Paragraph(f'<font color="#c2410c"><b>Takeaway&nbsp;&nbsp;</b></font>{text}',
                  takeaway)
...
def cover(canvas, doc):
    ...
    canvas.drawRightString(PAGE_W - MARGIN, PAGE_H - 1.7 * cm,
    ...
    canvas.drawString(MARGIN, 1.45 * cm,
    ...
    canvas.drawRightString(PAGE_W - MARGIN, 0.95 * cm,
                           "Synthetic dataset. Financial values are proxy estimates.")
    canvas.restoreState()
...
def build():
    ...
    A(Spacer(1, 7.2 * cm))
    A(Paragraph("What the data says", ParagraphStyle(
        "ct", fontName="Helvetica-Bold", fontSize=12, textColor=ACCENT)))
    A(Spacer(1, 4))
    A(Paragraph(
        ...
        "pool that disciplined intervention can recover.", ParagraphStyle(
            "cl", parent=body, fontSize=11, leading=16.5)))
    A(Spacer(1, 0.5 * cm))
    A(KpiStrip([
    ...
    toc.levelStyles = [
        ...
        ParagraphStyle("toc1", fontName="Helvetica", fontSize=9.5, leading=16,
    ...
    A(Paragraph(
        ...
        f"drift in replenishment performance, not a set of isolated shocks, and "
        f"the final month is the worst in the series.", lead))
    A(Paragraph(
        f"The exposure is concentrated on every axis the data exposes. Two "
        ...
        f"only two in the High risk tier. Both deliver late on a fifth to a third "
        f"of orders and carry lead-time variability near four days against a "
        f"sub-1.7-day norm for the rest of the base. At SKU-location grain the "
        f"concentration is sharper still: the worst 10% of the 480 scored pairs "
        f"carry {top10_share:.0f}% of all lost sales, and the worst 5% carry "
        f"{top5_share:.0f}%.", body))
    A(Paragraph(
        f"One product drives the top of the priority list. Of the 480 "
        ...
        f"supply or planning fault rather than a local one.", body))
    A(Paragraph(
        f"Inventory is misaligned rather than simply excessive. Madrid and Lyon "
        f"hold the least stock yet serve worst, while Lisbon and Porto carry the "
        f"most inventory value and post the best fill rates. The fill-rate spread "
        f"across warehouses is {fill_spread:.1f} points and tracks placement, not "
        f"total volume. At the same time {dos_over30:.0f}% of SKUs hold 30 or more "
        f"days of supply against a {dos_median:.0f}-day median, so capital is "
        f"trapped in slow lines while fast lines run thin.", body))
    A(Paragraph(
        f"Disciplined action on the ranked priorities supports an estimated "
        ...
        f"recommendations in Section 9 sequence the work from supplier remediation "
        f"to inventory rebalancing, ordered by return and dependency.", body))
    A(Spacer(1, 8))
    A(takeaway_box(
    ...
    A(Paragraph(
        ...
        "the product-warehouse grain, producing roughly 351,000 daily rows.", body))
    A(Paragraph(
        ...
        "and to size the value of fixing them.", body))
    A(Paragraph("The analysis answers five questions:", body))
    for p in bullets([
    ...
    A(Paragraph(
        ...
        "be read against a precise definition rather than an intuition.", body))
    ...
    A(Paragraph(
        ...
        "findings separate a stockout problem from an overstock problem.", body))
    A(Spacer(1, 4))
    A(data_table([
    ...
    A(Paragraph(
        ...
        f"loss, not a claim that all of it is reachable.", body))
    ...
    A(Paragraph(
        ...
        "sliding system is absorbed rather than converted to service.", body))
    ...
    A(Paragraph(
        ...
        "keeps the plan from over-investing in inventory.", body))
    ...
    A(Paragraph(
        ...
        "preconditions for the later ones to pay back.", body))
    A(Spacer(1, 8))
    A(takeaway_box(
    ...
    A(Paragraph(
        ...
        "and the last sizes the recoverable pool.", body))
    A(Spacer(1, 6))

    # ---- 5.1 service decline ----
    A(H2("5.1  Service quality is on a two-year decline"))
    A(Paragraph(
        ...
        "recovering toward the mean.", body))
    for it in chart("02_service_level_trend.png",
    ...
    A(Paragraph(
        ...
        "final months. A declining baseline matters for planning: inventory added "
        "without fixing replenishment discipline will be absorbed by the drift "
        "rather than converted into service, so stabilising the trend is a "
        "precondition for any inventory investment to pay back.", body))
    for it in chart("06_stockout_lost_sales_trend.png",
    ...
    A(H2("5.2  Two suppliers carry four fifths of the exposure"))
    A(Paragraph(
        ...
        f"relationship is clear: unreliable lead times, not low order fill, drive "
        f"the lost sales, since both high-risk suppliers still ship more than 92% "
        f"of ordered units once they arrive.", body))
    for it in chart("04_supplier_reliability.png",
    ...
    A(Paragraph(
        ...
        "high-volume lane than a thin one.", body))
    for it in chart("10_supplier_ontime_ranking.png",
                    "Figure 5. On-time delivery rate by supplier against the 80% service line."):
        A(it)
    A(takeaway_box(
        f"Supplier 2 and Supplier 3 are the single highest-return intervention. "
        f"Expedite constrained lanes and renegotiate lead-time variability before "
        f"any broad safety-stock increase."))
    A(PageBreak())

    # ---- 5.3 warehouse misalignment ----
    A(H2("5.3  Inventory is misaligned, not uniformly excessive"))
    A(Paragraph(
        ...
        f"already strong and is thin where it is failing.", body))
    for it in chart("03_warehouse_service_inventory.png",
    ...
    A(Paragraph(
        ...
        f"a uniform glut.", body))
    for it in chart("09_days_of_supply_distribution.png",
    ...
    A(H2("5.4  Half the value pool sits in one category"))
    A(Paragraph(
        ...
        f"applied across the catalogue.", body))
    for it in chart("01_opportunity_by_category.png",
                    "Figure 8. Estimated 12-month opportunity by category."):
        A(it)
    A(Paragraph(
        "Health does not lead the pool because it is large; it leads because it "
        "serves worst. Health posts the lowest fill rate of any category at 93.3% "
        "and the highest lost-sales value at €9.60M, despite holding among the most "
        "inventory value. Pet Care is second worst on both. The categories that "
        ...
        "priority order.", body))
    for it in chart("13_fill_rate_by_category.png",
    ...
    A(Paragraph(
        ...
        "product level rather than at a single site.", body))
    for it in chart("12_segment_risk_heatmap.png",
    ...
    A(H2("5.5  The loss is concentrated in a few SKU-location pairs"))
    A(Paragraph(
        ...
        f"without touching the long tail of pairs that lose almost nothing.", body))
    for it in chart("07_lost_sales_concentration.png",
    ...
    A(Paragraph(
        ...
        body))
    for it in chart("05_top_sku_opportunity.png",
    ...
    A(Paragraph(
        ...
        "the textbook signature of a placement and policy gap rather than a budget "
        "shortage.", body))
    for it in chart("08_abc_class_cohort.png",
    ...
    # ---- 5.6 value pool ----
    A(H2("5.6  The recoverable pool is real but partial"))
    A(Paragraph(
        ...
        f"be converted once service is restored.", body))
    for it in chart("11_opportunity_bridge.png",
    ...
    A(Paragraph(
        ...
        f"secondary benefit from trimming the slow-moving tail.", body))
    A(takeaway_box(
    ...
    A(Paragraph(
        ...
        "stable than the absolute totals.", body))
    ...
    A(Paragraph(
        ...
        "service-side value rather than a net benefit.", body))
    A(Spacer(1, 6))
    A(takeaway_box(
    ...
    A(Paragraph(
        ...
        f"concentrated intervention set within the {eur(total_lost)} lost-sales exposure. The declining fill-rate "
        ...
        f"bulk of the value and require no inventory investment.", body))
    A(Spacer(1, 4))
    ...
    for title, desc, hor, pr in recs:
        cell = Paragraph(
            f"<b>{title}</b><br/><font size=8.5 color='#56616c'>{desc}</font>",
    ...
    A(Paragraph(
        ...
        f"released capital, with the first two actions carrying most of it.", body))
    A(Spacer(1, 6))
    A(takeaway_box(
        "Do the two no-cost fixes first. Supplier discipline and one product "
        "recovery plan reach most of the value before any inventory moves."))
    A(PageBreak())
    ...
    A(Paragraph(
        ...
        "figures are reproducible from the source pipeline.", body))
    ...
    for _, r in wh.sort_values("fill_rate").iterrows():
        rows.append([
            r.warehouse_name, r.region, f"{r.fill_rate*100:.1f}%",
            f"{r.stockout_rate*100:.1f}%",
            f"{r.average_days_of_supply:.1f}", eur(r.inventory_value)])
    A(data_table(rows, [4.0 * cm, 3.6 * cm, 1.7 * cm, 1.9 * cm, 1.6 * cm, 2.3 * cm],
                 align_right=[2, 3, 4, 5]))
    A(Spacer(1, 14))
    ...
    for _, r in cat.iterrows():
        ...
        rows.append([r.entity_name, eur(r.opportunity_total_12m_proxy),
                     f"{r.opportunity_total_12m_proxy/cat_pool*100:.0f}%",
                     f"{cf:.1f}%"])
    A(data_table(rows, [4.6 * cm, 4.6 * cm, 2.2 * cm, CW - 11.4 * cm],
                 align_right=[1, 2, 3]))
    A(Spacer(1, 14))
    ...
    for c in ["A", "B", "C"]:
        rows.append([f"Class {c}", f"{int(abc.loc[c,'n'])}",
                     f"{abc.loc[c,'inv_sh']:.0f}%", f"{abc.loc[c,'lost_sh']:.0f}%"])
    A(data_table(rows, [3.5 * cm, 2.5 * cm, 5.0 * cm, CW - 11.0 * cm],
                 align_right=[1, 2, 3]))
    A(Spacer(1, 14))
    ...
    for _, r in segr.iterrows():
        rows.append([r.segment_id, f"{r.governance_priority_score:.0f}",
                     r.risk_tier, r.main_risk_driver, f"{r.fill_rate*100:.1f}%"])
    A(data_table(rows, [6.0 * cm, 1.6 * cm, 1.9 * cm, 4.0 * cm, CW - 13.5 * cm],
                 align_right=[1, 4], fs=7.6))
    A(PageBreak())

    A(H2("H. Monthly service KPIs (24 months)"))
    A(Paragraph(
        "Network fill rate, stockout rate, and lost-sales value by month over the "
        "full observation window. This is the series behind Figures 1 and 2.", small))
    A(Spacer(1, 4))
    rows = [["Month", "Fill rate", "Stockout rate", "Lost-sales value"]]
    for m, r in monthly.iterrows():
        rows.append([str(m), f"{r.fill:.1f}%", f"{r.so*100:.1f}%", eur(r.lost)])
    A(data_table(rows, [3.5 * cm, 3.5 * cm, 4.0 * cm, CW - 11.0 * cm],
                 align_right=[1, 2, 3], fs=8))
    A(Spacer(1, 14))
    ...
    A(Paragraph(
        ...
        "fast-moving Class A items.", small))
    A(Spacer(1, 4))
    ...
    for _, r in over.iterrows():
        rows.append([r.product_name, r.category, r.abc_class,
