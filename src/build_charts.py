...
# ---- Shared style -----------------------------------------------------------
INK = "#1d2733"          # primary text / strong elements
MUTED = "#9aa6b2"        # neutral bars and secondary marks
GRID = "#e4e8ec"         # gridlines
ACCENT = "#c2410c"       # single accent for the element that carries the point
ACCENT_SOFT = "#f0b49a"  # accent tint
...
def _save(fig, name, caption):
    fig.text(0.0, -0.02, caption, ha="left", va="top", fontsize=8.5,
...
def chart_opportunity_by_category():
    ...
    d = (p[p.entity_type == "Category"]
         .sort_values("opportunity_total_12m_proxy"))
    total = d["opportunity_total_12m_proxy"].sum()
    colors = [ACCENT if v == d["opportunity_total_12m_proxy"].max() else MUTED
              for v in d["opportunity_total_12m_proxy"]]

    fig, ax = plt.subplots(figsize=(8.6, 5.0))
    bars = ax.barh(d["entity_name"], d["opportunity_total_12m_proxy"],
                   color=colors, zorder=3, height=0.66)
    ...
    for bar, val in zip(bars, d["opportunity_total_12m_proxy"]):
        ax.text(val + total * 0.006, bar.get_y() + bar.get_height() / 2,
                f"{_eur(val)}  ({val/total*100:.0f}%)", va="center",
                ha="left", fontsize=10,
                color=INK if val == d["opportunity_total_12m_proxy"].max() else "#56616c",
                fontweight="bold" if val == d["opportunity_total_12m_proxy"].max() else "normal")
    _titles(ax, "Health holds nearly half of the recoverable value pool",
            "Estimated 12-month opportunity by category "
            "(margin recovery + working-capital release)")
    fig.subplots_adjust(top=0.84)
    _save(fig, "01_opportunity_by_category.png",
...
def chart_service_trend():
    df = pd.read_csv(PROC / "daily_product_warehouse_metrics.csv",
                     usecols=["date", "units_demanded", "units_fulfilled",
                              "lost_sales_revenue"], parse_dates=["date"])
    df["m"] = df["date"].dt.to_period("M").dt.to_timestamp()
    g = (df.groupby("m")
         .agg(dem=("units_demanded", "sum"),
...
def chart_warehouse_quadrant():
    ...
    y = w["fill_rate"] * 100
    sizes = w["inventory_value"] / w["inventory_value"].max() * 900 + 140
    ...
    for _, r in w.iterrows():
        ...
        ax.scatter(r["average_days_of_supply"], r["fill_rate"] * 100,
                   s=r["inventory_value"] / w["inventory_value"].max() * 900 + 140,
                   color=col, alpha=0.85, zorder=3, edgecolor="white", linewidth=1.5)
        ax.annotate(f"{r['warehouse_name']}\n{r['fill_rate']*100:.1f}% fill  "
                    f"{_eur(r['inventory_value'])} inv",
                    (r["average_days_of_supply"], r["fill_rate"] * 100),
                    textcoords="offset points", xytext=(11, 6), fontsize=9,
                    color=INK)
    ...
    _titles(ax, "Madrid and Lyon run leaner yet serve worse",
            "Warehouse fill rate vs days of supply. Bubble area = inventory value.")
    fig.subplots_adjust(top=0.85)
    _save(fig, "03_warehouse_service_inventory.png",
...
def chart_supplier_reliability():
    ...
    for _, r in s.iterrows():
        ...
        ax.scatter(r["on_time_delivery_rate"] * 100, r["lead_time_variability"],
                   s=r["lost_sales_revenue"] / s["lost_sales_revenue"].max() * 1100 + 90,
                   color=col, alpha=0.82, zorder=3, edgecolor="white", linewidth=1.4)
        if high:
            ax.annotate(f"{r['supplier_name']}\n{r['on_time_delivery_rate']*100:.0f}% on-time  "
                        f"{_eur(r['lost_sales_revenue'])} lost",
                        (r["on_time_delivery_rate"] * 100, r["lead_time_variability"]),
                        textcoords="offset points", xytext=(12, 4), fontsize=9.2,
    ...
    _titles(ax, "Two suppliers concentrate the delivery risk",
            "Lead-time variability vs on-time delivery. Bubble area = lost-sales value.")
    fig.subplots_adjust(top=0.85)
    _save(fig, "04_supplier_reliability.png",
...
def chart_top_sku():
    ...
    fig, ax = plt.subplots(figsize=(8.6, 5.4))
    bars = ax.barh(d["entity_name"], d["opportunity_total_12m_proxy"],
                   color=colors, zorder=3, height=0.68)
    _clean(ax, grid_axis="x")
    ax.xaxis.set_major_formatter(mticker.FuncFormatter(lambda x, _: _eur(x)))
    ax.set_xlim(0, d["opportunity_total_12m_proxy"].max() * 1.18)
    for bar, val in zip(bars, d["opportunity_total_12m_proxy"]):
        ax.text(val + d["opportunity_total_12m_proxy"].max() * 0.012,
                bar.get_y() + bar.get_height() / 2, _eur(val),
                va="center", ha="left", fontsize=9.5, color="#56616c")
    _titles(ax, "One product across four warehouses leads the list",
            "Top 10 SKU-warehouse pairs by 12-month opportunity. "
            "Accent = Health Product 16.")
    fig.subplots_adjust(top=0.85)
    _save(fig, "05_top_sku_opportunity.png",
...
def chart_lost_sales_concentration():
    ...
    ax.plot([0, 100], [0, 100], color=MUTED, linewidth=1.1, linestyle=(0, (4, 4)),
            zorder=2)
    _clean(ax, grid_axis="both")
    ax.set_xlim(0, 100)
    ax.set_ylim(0, 100)
...
def chart_abc_cohort():
    ...
    y = range(len(g))
    h = 0.36
    ax.barh([i + h / 2 for i in y], inv_sh, height=h, color=MUTED, zorder=3,
            label="Inventory value")
    ax.barh([i - h / 2 for i in y], lost_sh, height=h, color=ACCENT, zorder=3,
    ...
    for i, c in enumerate(g.index):
        ax.text(inv_sh[c] + 1, i + h / 2, f"{inv_sh[c]:.0f}%", va="center",
                fontsize=9.5, color="#56616c")
        ax.text(lost_sh[c] + 1, i - h / 2, f"{lost_sh[c]:.0f}%", va="center",
                fontsize=9.5, color=ACCENT, fontweight="bold")
    ax.legend(loc="lower right", frameon=False, fontsize=9.5)
    _titles(ax, "Class A drives lost sales out of proportion to its count",
            "Share of inventory value and lost-sales exposure by ABC class.")
    fig.subplots_adjust(top=0.85)
    _save(fig, "08_abc_class_cohort.png",
...
def chart_dos_distribution():
    ...
    fig, ax = plt.subplots(figsize=(8.6, 5.0))
    n, bins, patches = ax.hist(d, bins=24, color=MUTED, zorder=3,
    ...
    ax.annotate(f"Median {med:.0f} days", (med, max(n) * 0.92),
    ...
    _titles(ax, f"A long tail of overstocked SKUs sits past 30 days",
            f"Distribution of average days of supply across 120 SKUs. "
            f"{over:.0f}% hold 30+ days (accent).")
    fig.subplots_adjust(top=0.85)
    _save(fig, "09_days_of_supply_distribution.png",
...
def chart_supplier_ranking():
    s = pd.read_csv(PROC / "supplier_performance_summary.csv").sort_values(
        "on_time_delivery_rate")
    colors = [ACCENT if v < 0.80 else MUTED for v in s["on_time_delivery_rate"]]
    fig, ax = plt.subplots(figsize=(8.6, 5.6))
    bars = ax.barh(s["supplier_name"], s["on_time_delivery_rate"] * 100,
                   color=colors, zorder=3, height=0.66)
    ax.axvline(80, color=INK, linewidth=1.0, linestyle=(0, (4, 3)), zorder=4)
    _clean(ax, grid_axis="x")
    ax.set_xlim(0, 100)
    ax.xaxis.set_major_formatter(mticker.PercentFormatter(decimals=0))
    for bar, v in zip(bars, s["on_time_delivery_rate"]):
        ax.text(v * 100 - 1.5, bar.get_y() + bar.get_height() / 2,
                f"{v*100:.0f}%", va="center", ha="right", fontsize=9,
                color="white", fontweight="bold")
    ax.annotate("80% service line", (80, len(s) - 0.4),
                textcoords="offset points", xytext=(6, 0), fontsize=9,
                color=INK)
    _titles(ax, "Three suppliers fall below the 80% on-time line",
            "On-time delivery rate by supplier. Accent marks below-threshold suppliers.")
    fig.subplots_adjust(top=0.88)
    _save(fig, "10_supplier_ontime_ranking.png",
...
def chart_opportunity_bridge():
    ...
    ax.plot([0.31, 0.69], [margin, margin], color="#9aa6b2", linewidth=0.9,
            linestyle=(0, (2, 2)), zorder=2)
    ax.plot([1.31, 1.69], [total, total], color="#9aa6b2", linewidth=0.9,
            linestyle=(0, (2, 2)), zorder=2)
    _clean(ax, grid_axis="y")
    ax.set_xticks([0, 1, 2])
    ...
    ax.text(0, margin + total * 0.02, _eur(margin), ha="center", fontsize=10.5,
            fontweight="bold", color=INK)
    ax.text(1, margin + wc + total * 0.02, f"+{_eur(wc)}", ha="center",
            fontsize=10.5, fontweight="bold", color="#56616c")
    ax.text(2, total + total * 0.02, _eur(total), ha="center", fontsize=10.5,
            fontweight="bold", color=INK)
    _titles(ax, "Margin recovery dominates the value pool",
            f"Composition of the estimated 12-month opportunity ({_eur(total)} proxy).")
    fig.subplots_adjust(top=0.85)
    _save(fig, "11_opportunity_bridge.png",
...
def chart_segment_heatmap():
    ...
    piv = seg.pivot_table(index="category", columns="region",
                          values="governance_priority_score")
    ...
    im = ax.imshow(data, cmap=cmap, aspect="auto", vmin=np.nanmin(data),
                   vmax=np.nanmax(data))
    ...
    _titles(ax, "Health and Pet Care segments carry the highest risk",
            "Governance priority score by category and region (0-100). "
            "Darker = higher priority.")
    fig.subplots_adjust(top=0.88)
    _save(fig, "12_segment_risk_heatmap.png",
...
def chart_category_fill():
    ...
    g = (df.groupby("category")
         .agg(dem=("units_demanded", "sum"), ful=("units_fulfilled", "sum"),
              lost=("lost_sales_revenue", "sum")).reset_index())
    ...
    ax.annotate(f"Network {net:.1f}%", (net, len(g) - 0.4),
                textcoords="offset points", xytext=(6, 0), fontsize=9, color=INK)
    for bar, v in zip(bars, g["fill"]):
        ax.text(v - 0.3, bar.get_y() + bar.get_height() / 2, f"{v:.1f}%",
                va="center", ha="right", fontsize=9, color="white",
                fontweight="bold")
    _titles(ax, "Health serves worst despite holding the most value",
            "Fill rate by product category over the full window.")
    fig.subplots_adjust(top=0.86)
    _save(fig, "13_fill_rate_by_category.png",
...
def chart_before_after():
    df = pd.read_csv(PROC / "daily_product_warehouse_metrics.csv",
                     usecols=["date", "warehouse_id", "units_demanded",
                              "units_fulfilled"], parse_dates=["date"])
    ...
    def fr(d):
        g = d.groupby("warehouse_id").agg(dem=("units_demanded", "sum"),
                                          ful=("units_fulfilled", "sum"))
        return (g["ful"] / g["dem"] * 100)
    e, l = fr(early), fr(late)
    m = (pd.DataFrame({"early": e, "late": l}).join(wh.set_index("warehouse_id"))
         .sort_values("early"))

    fig, ax = plt.subplots(figsize=(8.6, 5.2))
    y = range(len(m))
    for i, (_, r) in enumerate(m.iterrows()):
        ax.plot([r["early"], r["late"]], [i, i], color="#c9d0d6", linewidth=2.4,
                zorder=2)
        ax.scatter(r["early"], i, color=MUTED, s=90, zorder=3)
        ax.scatter(r["late"], i, color=ACCENT, s=90, zorder=4)
        ax.annotate(f"{r['late']-r['early']:+.1f} pts",
    ...
    ax.legend(loc="lower right", frameon=False, fontsize=9.5)
    _titles(ax, "Every warehouse serves worse than it did at the start",
            "Fill rate, first 90 days vs last 90 days of the window.")
    fig.subplots_adjust(top=0.86)
    _save(fig, "14_before_after_fill_rate.png",
